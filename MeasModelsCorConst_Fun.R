...
#run on computing cluster - example script for multiple regression outcome models. Run for each outcome, effects on highly correlated factors constrained 
outcome_name <- args[1]
...
aothr.2 == ain.2
  ...
  saveRDS(output, paste0(output_filepath,"/",jobname,"_const_cor.rds"))
  
  if(return_fit == TRUE) {saveRDS(fit, paste0(output_filepath,"/",jobname,"_const_cor_fitobj.rds"))}
